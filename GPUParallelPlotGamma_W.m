...
function output = GPUParallelPlotGamma_W(W,alphabetSize,speeds,intervalCount,lenR,simCount)
    if length(speeds) ~= alphabetSize
        disp("Error!");
        return
    end
    gammaGraph = gpuArray(zeros); %Array of gamma values.
    xAxis = gpuArray(zeros); %Array of x values (rho).
    xAxis(1)=0;
    ...
    for i = 1:intervalCount
        xAxis(i+1) = i*step;
        gammaGraph(i+1) = GPUParallelEstimateGamma_W(W,alphabetSize,i*step,lenR,simCount);
    end
    ...
    for i = 1:(alphabetSize-1)
        ...
        for j = 1:intervalCount
            ...
            gammaGraph(i*intervalCount+j+1) = GPUParallelEstimateGamma_W(W,alphabetSize,a+j*step,lenR,simCount);
        end
    end
    ...
    for j = 1:intervalCount
        ...
        gammaGraph(alphabetSize*intervalCount+j+1) = GPUParallelEstimateGamma_W(W,alphabetSize,a+j*step,lenR,simCount);
    end
    toc;
