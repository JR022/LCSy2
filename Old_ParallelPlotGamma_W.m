...
function Old_ParallelPlotGamma_W(W,alphabetSize,speeds,intervalCount,lenR,simCount)
    %Initialise text file
    fileName = strcat("Old_PlotGamma_W--",string(datetime('now','TimeZone','local','Format','d-MMM-y_HH-mm-ss_Z')));
    ...
    for j = 1:intervalCount
        ...
        gammaGraph(alphabetSize*intervalCount+j+1) = ParallelEstimateGamma_W(W,alphabetSize,a+j*step,lenR,simCount);
        fprintf(myFile,"(%.10f:%.10f),\n",xAxis(alphabetSize*intervalCount+j+1),gammaGraph(alphabetSize*intervalCount+j+1));
    end
    time = toc;
