...
function gamma_W = ParallelEstimateGamma_W(W,alphabetSize,rho,lenR,simCount)
    tic
    if ischar(W)
        tmp = zeros;
        for i = 1:length(W)
            tmp(i) = str2double(W(i));
        end
        W = tmp;
        clearvars tmp;
    end
    ...
    gamma_W = vpa(LCStotal/simCount/lenR); %Division by lenR is needed to get \gamma_W
    toc
end
